...
import {
    ...
    Alert, // Added for modal logic
} from 'react-native';
// Keep Image from expo-image for category items
import { Image } from 'expo-image';
import { rootApi } from './axiosInstance';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { useRouter } from 'expo-router';
// 1. ADD EXPO IMAGE PICKER SIMULATION AND FILE HANDLING UTILITIES
import * as ImagePicker from 'expo-image-picker'; 
import axios from 'axios'; // Used for FormData type check logic
...
// Placeholder for menu category images 
...
// Reusing placeholders for Boritto and Taco
...
// NEW LOCAL IMAGE FOR SPECIAL DISH
const SPECIAL_DISH_IMAGE = require('../assets/images/cheeseburger.jpg');

// FIX: todaysSpecialData defined here, before the component TodaysSpecialCard
...
// MAPPING to link API names to local image paths/URLs
const IMAGE_MAP = {
    'Pizza': PIZZA_IMAGE,
...
const SearchBar = ({ onSearchClose, isMobile }) => (
...
const CategoryItem = ({ name, image, isActive, onPress }) => (
    <TouchableOpacity 
...
const LogoutDropdown = ({ positionStyle, onLogout, onClose }) => (
...
const TotalSaleCard = ({ title, value, trendColor }) => (
...
const SmallAnalyticsCard = ({ title, value, trendColor }) => (
...
// 🚩 Component can now safely access todaysSpecialData
...
// 🚩 MODIFIED COMPONENT: Offer Card (Added Edit and Delete buttons)
// 🚩 MODIFIED COMPONENT: Offer Card (Added onDelete prop and handler)
const OfferCard = ({ title, offerDetail, image, isDarkMode, offerData, onEdit, onDelete }) => (
    <View style={[styles.offerCardItem, isDarkMode && darkStyles.sectionContainer]}>
        ...
        <View style={styles.specialFooter}>
            ...
            <View style={styles.offerActionButtons}>
                ...
                {/* 🚩 UPDATED: Attached onPress to onDelete */}
                <TouchableOpacity 
...
// 🚩 MODIFIED SECTION: Offers Scroll Section (Accepts onAddOffer prop)
// 🚩 MODIFIED SECTION: Offers Scroll Section (Added onDeleteOffer prop)
const OffersScrollSection = ({ offers, isLoading, isDarkMode, isWeb, onAddOffer, onEditOffer, onDeleteOffer }) => (
    <View 
        style={[
            ...
            >
                {offers.map((offer) => (
                    <OfferCard
                        ...
                        offerData={offer}
                        onEdit={onEditOffer}
                        onDelete={onDeleteOffer} // 🚩 Pass the delete handler here
                    />
                ))}
            </ScrollView>
        )}
...
const OrderReportsTable = ({ orders, totalCount, visibleCount, onShowMore, isLoading, isDarkMode }) => {
    // Static data placeholder for report table (can be removed if API fetches all data)
    const orderReportsData = [
        { id: 1, food: 'Fish Burger', customer: 'Jessica Taylor', date: '25/10/2024', price: '$30.00', status: 'Pending', image: 'https://picsum.photos/id/1010/50/50' },
        { id: 2, food: 'Pepperoni Pizza', customer: 'Jane Cooper', date: '20/11/2024', price: '$57.20', status: 'Completed', image: 'https://picsum.photos/id/238/50/50' },
        { id: 3, food: 'Hot Dog', customer: 'Ollivia Anderson', date: '18/10/2024', price: '$40.00', status: 'Pending', image: 'https://picsum.photos/id/257/50/50' },
        { id: 4, food: 'Nachos', customer: 'Sophia Garcia', date: '02/08/2024', price: '$30.25', status: 'Completed', image: 'https://picsum.photos/id/1020/50/50' },
        { id: 6, food: 'Japanese Ramen', customer: 'David Wilson', date: '26/07/2024', price: '$71.01', status: 'Completed', image: 'https://picsum.photos/id/1037/50/50' },
    ];
    // Uses props.orders data if available
    ...
    return (
        <View style={[styles.orderReportsContainer, isDarkMode && darkStyles.orderReportsContainer]}>
            ...
            {!isLoading && orders.length === 0 && (
                <View style={{ padding: 20, alignItems: 'center' }}>
                    ...
                    <Text style={{ color: isDarkMode ? '#aaa' : '#999', fontSize: 12, marginTop: 5 }}>
                        (Note: API requires Admin/Manager role. If you see 403 in the console, check user roles.)
                    </Text>
                </View>
            )}
            ...
            {/* 🎯 FIXED: Changed visibleOrdersCount to visibleCount */}
...
const TrendingCard = ({ name, price, description, sales, rating, tag, image, isDarkMode }) => (
    // Static data placeholder for trending orders
...
const TrendingOrdersSection = ({ isWeb, isDarkMode }) => {
    // Static data is declared globally, available here
...
// --- NAV/HEADER COMPONENTS (MODIFIED) ---
const WebHeader = ({ handleLogout, isDarkMode }) => {
    const [isProfileDropdownOpen, setIsProfileDropdownOpen] = useState(false);
    const [isSearchOpen, setIsSearchOpen] = useState(false); // 🚩 Search State
    
    const profilePicRef = useRef(null);
    const [profilePicLayout, setProfilePicLayout] = useState(null);

    const toggleProfileDropdown = () => {
        setIsProfileDropdownOpen(prev => !prev);
    };

    const onProfilePicLayout = (event) => {
        if (!profilePicLayout) {
            profilePicRef.current.measureInWindow((x, y, width, height) => {
            ...
            <View style={[styles.headerCard, isDarkMode && darkStyles.headerCard]}>
                ...
                <View style={styles.headerButtonGroup}>
                    <View style={styles.iconButtons}>
                        {/* 🚩 Web: If Search is open, show SearchBar; otherwise, show the Search icon */}
                        {isSearchOpen ? (
                        ...
                        {/* Notification and Dark Mode buttons */}
                        {!isSearchOpen && (
                            <>
                                ...
                                <TouchableOpacity style={[styles.iconButton, isDarkMode && darkStyles.iconButton]} onPress={() => AdminPage.toggleDarkMode()}>
...
const MobileHeader = ({ onHamburgerPress, handleLogout, isDarkMode }) => {
    const [isProfileDropdownOpen, setIsProfileDropdownOpen] = useState(false);
    const [isSearchOpen, setIsSearchOpen] = useState(false); // 🚩 Search State
    ...
    return (
        <View style={{zIndex: 9000}}>
            {/* 🚩 Mobile: Show Full Search Bar if open */}
            {isSearchOpen ? (
                ...
                <View style={[styles.mobileHeaderContainer, isDarkMode && darkStyles.mobileHeaderContainer]}>
                    ...
                    <View style={styles.mobileHeaderRight}>
                        {/* 🚩 Mobile: Icon to open search bar */}
...
const SidebarContent = ({ isDarkMode }) => (
    <>
...
// --- END NAV/HEADER COMPONENTS ---

// 🚩 NEW COMPONENT: OfferModal (MODIFIED for Add/Edit logic)
const OfferModal = ({ isVisible, onClose, onSave, offerData, isDarkMode }) => {
    const isEditMode = !!offerData;
    
    // Initialize state from props (for Edit) or default (for Add)
    ...
    // Sync internal state when props change (for Edit mode pre-fill)
    useEffect(() => {
        if (offerData) {
            setTitle(offerData.title || '');
            setDescription(offerData.description || '');
            setImageUri(offerData.imageUrl || null);
            setImageFile(null); // Clear file for edit, as we only need it if the user selects a *new* image
        } else {
            setTitle('');
            setDescription('');
            setImageUri(null);
    ...
    // 🚩 IMAGE PICKER FUNCTION (MODIFIED FOR WEB FILE OBJECT)
    const pickImage = async () => {
        ...
        if (!pickerResult.canceled) {
            ...
            if (Platform.OS === 'web') {
                // On Web, store the native File object for direct FormData appending
                setImageFile(pickerResult.assets[0].file);
            } else {
                // On Native, nullify the file object as the URI descriptor method will be used
                setImageFile(null); 
            }
        }
    };
    ...
    const handleSave = () => {
        ...
        // 🚩 PASS ID, TEXT DATA, IMAGE URI, AND FILE OBJECT
        onSave(offerData?.id, title, description, imageUri, imageFile);
    };
    
    const handleClose = () => {
        // Parent component will handle clearing state via prop update
        onClose();
    };
...
export default function AdminPage() {
    ...
    const [modalOfferData, setModalOfferData] = useState(null); // {id, title, description, imageUrl} or null for Add
    ...
    // 🚩 MODIFIED: Function to handle 'Add New Offer' button click - opens modal
    const handleAddOffer = () => {
        setModalOfferData(null); // Set to null for ADD mode
        setIsModalVisible(true);
    };

    // 🚩 NEW: Function to handle 'Edit Offer' button click - opens modal with data
    const handleEditOffer = (offer) => {
        setModalOfferData(offer); // Pass data for EDIT mode
        setIsModalVisible(true);
    };
    ...
    const handleModalClose = useCallback(() => {
        ...
        setModalOfferData(null); // Clear data when closing
    }, []);

    // 🚩 MODIFIED: Function to handle saving or updating the offer

// 🚩 NEW: Handle Delete Logic
    const handleDeleteOffer = async (id) => {
        // Logic to perform the API Delete
        const performDelete = async () => {
            try {
                // Call the API
                await rootApi.delete(`/offer/deleteOffer/${id}`);
                
                // Update Local State (Remove the item from the list immediately)
                setOffers(prevOffers => prevOffers.filter(offer => offer.id !== id));
                
                // Show success message (Optional, as the UI updates immediately)
                if (Platform.OS !== 'web') {
        ...
        // Confirmation Dialog (Web vs Mobile)
        if (Platform.OS === 'web') {
  ...
  // 🚩 FIXED FUNCTION: Handles specific Controller Keys and avoids empty FormData on Android
const handleSaveOrUpdateOffer = async (id, title, description, imageUri, imageFile) => {
    // 1. Determine if this is an update and if the image is actually new
    const isUpdate = !!id;
    const isImageNew = Platform.OS === 'web' 
        ? !!imageFile 
        : (imageUri && !imageUri.startsWith('http')); // If it starts with http, it's an existing remote image
    ...
    // 2. Determine the correct Key for the file (Matches your backend)
    ...
    console.log(`Sending ${action} request via ${method.toUpperCase()} to: ${endpoint}`);

    // 5. Prepare Request Body and Headers
    let requestBody = null;
    let requestHeaders = {};

    if (isImageNew) {
        // CASE A: We have a NEW image to upload. Use FormData.
        ...
        // --- ANDROID / IOS LOGIC ---
            ...
            // React Native needs these 3 fields for file uploads
            // @ts-ignore
        ...
        // IMPORTANT: Only force this header if we actually have FormData
        requestHeaders = { 'Content-Type': 'multipart/form-data' };
    } else {
        // CASE B: Text-only update. 
        // DO NOT send empty FormData on Android (causes 403 Error).
        // Send null body; backend will read Query Params.
        requestBody = null;
        // We let Axios use the default content type (usually application/json)
    }
    ...
    try {
        ...
        if (error.response && error.response.status === 403) {
             ...
             Alert.alert("Authorization Failed", "The server rejected the request format. Check logs.");
        } else {
    ...
    // --- API FETCH EFFECT (ORDERS) ---
    useEffect(() => {
        const fetchOrders = async () => {
            setIsLoadingOrders(true);
            const userRolesString = await AsyncStorage.getItem('roles');
            const isAuthorized = userRolesString && userRolesString.includes('ROLE_ADMIN');

            if (!isAuthorized) {
                    console.warn("Unauthorized access attempt: User does not have ROLE_ADMIN.");
        ...
        const fetchOffers = async () => {
            ...
            try {
               ...
               let response = await rootApi.get(`/offer/getActiveOffers`)
                // The API response is expected to be an array of offers
    ...
    return (
        <View style={[isWeb ? styles.containerWebLayout : styles.containerMobile, isDarkMode && darkStyles.containerWebLayout]}>
            ...
            <View style={[styles.mainContentArea, isDarkMode && darkStyles.mainContentArea]}>
                {!isWeb && <MobileHeader onHamburgerPress={toggleDrawer} handleLogout={handleLogout} isDarkMode={isDarkMode} />}
                ...
                >
                    {isWeb && <WebHeader handleLogout={handleLogout} isDarkMode={isDarkMode} />}
                    ...
                    {isWeb ? (
                        ...
                        <>
                            ...
                            {/* 🚩 WEB: Offer Card below Order Report Card (full width) */}
                            <OffersScrollSection 
                                offers={offers} 
                                isLoading={isLoadingOffers} 
                                isDarkMode={isDarkMode} 
                                isWeb={isWeb} 
                                onAddOffer={handleAddOffer} // Attached handleAddOffer
                                onEditOffer={handleEditOffer} // Attached handleEditOffer
                            ...
                            <View style={styles.dynamicContentContainerMobile}>
                                ...
                                <View style={styles.reportsAndSpecialsContainerMobile}>
                                    ...
                                    <View style={styles.salesAndSpecialColumnMobile}>
                                        <SalesFiguresChart />
                                        <TodaysSpecialCard />
                                        {/* 🚩 ANDROID: Offer Card below Sales Figures Card */}
                                        <OffersScrollSection 
                                            offers={offers} 
                                            isLoading={isLoadingOffers} 
                                            isDarkMode={isDarkMode} 
            ...
            {/* 🚩 RENDER ADD/EDIT OFFER MODAL */}
            <OfferModal
                isVisible={isModalVisible}
                onClose={handleModalClose}
                onSave={handleSaveOrUpdateOffer} // Unified handler for Save/Update
                offerData={modalOfferData} // Pass initial data (null for Add, object for Edit)
                isDarkMode={isDarkMode}
            />
        </View>
    );
