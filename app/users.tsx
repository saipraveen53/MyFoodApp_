        ...
        <View style={styles.headerCardWrapper}> 
            <View style={[styles.headerCard, isDarkMode && darkStyles.headerCard]}>
                <View style={styles.headerTitleGroup}>
                    <Text style={[styles.headerTitle, isDarkMode && darkStyles.textPrimary]}>Users</Text>
                    <Text style={[styles.headerSubtitle, isDarkMode && darkStyles.textSecondary]}>Manage registered users</Text>
                </View>
...
const UsersTable = ({ users, isLoading, isDarkMode }: any) => {
    return (
        <View style={[styles.tableContainer, isDarkMode && darkStyles.tableContainer]}>
            <View style={styles.tableHeaderSection}>
                <Text style={[styles.sectionTitle, isDarkMode && darkStyles.textPrimary]}>All Users</Text>
            </View>
            
            {isLoading && (
                <View style={{ padding: 40, alignItems: 'center' }}>
                    <ActivityIndicator size="large" color="#FF9500" />
                    <Text style={{ color: isDarkMode ? '#aaa' : '#999', marginTop: 10 }}>Loading users...</Text>
                </View>
            )}
            
            {!isLoading && users.length === 0 && (
                <View style={{ padding: 40, alignItems: 'center' }}>
                    <Text style={{ color: isDarkMode ? '#aaa' : '#999', fontSize: 16 }}>No users found.</Text>
                </View>
            )}
                ...
                >
                    <View style={styles.table}> 
                        <View style={styles.tableHeader}>
                            ...
                            <Text style={[styles.headerCol, isDarkMode && darkStyles.textSecondary, styles.colName]}>Name</Text>
                            <Text style={[styles.headerCol, isDarkMode && darkStyles.textSecondary, styles.colEmail]}>Email</Text>
                            <Text style={[styles.headerCol, isDarkMode && darkStyles.textSecondary, styles.colPhone]}>Phone</Text>
                        </View>
                        ...
                        {users.map((item: any) => (
                            <View key={item.id} style={[styles.tableRow, isDarkMode && darkStyles.tableRow]}>
                                ...
                                <View style={[styles.dataCol, styles.colName]}>
                                    <View style={[styles.avatarPlaceholder, { backgroundColor: isDarkMode ? '#444' : '#eee' }]}>
                                        <Text style={{fontSize: 14, fontWeight:'bold', color: isDarkMode ? '#fff' : '#666'}}>
                                            {item.name ? item.name.charAt(0).toUpperCase() : '?'}
                                        </Text>
                                    </View>
                                    <Text style={[styles.nameText, isDarkMode && darkStyles.textPrimary]}>{item.name}</Text>
                                </View>
                                <Text style={[styles.dataCol, styles.colEmail, isDarkMode && darkStyles.textPrimary]}>{item.email}</Text>
                                <Text style={[styles.dataCol, styles.colPhone, isDarkMode && darkStyles.textPrimary]}>{item.phone}</Text>
                            </View>
                        ))}
                    </View>
                </ScrollView>
...
export default function UsersPage() {
    ...
    const fetchUsers = async () => {
        setLoading(true);
        try {
            const response = await rootApi.get('/user/allUsers');
            if (response.data) {
                setUsers(response.data);
            }
        } catch (error) {
            console.error("Failed to fetch users:", error);
            if(Platform.OS !== 'web') Alert.alert("Error", "Failed to fetch users.");
        } finally {
            setLoading(false);
        }
    };
...
const darkStyles = StyleSheet.create({
    ...
    tableContainer: { backgroundColor: DARK_CARD },
...
const styles = StyleSheet.create({
    ...
    // Column Widths
    colId: { width: 60 },
    colName: { flex: 1.5, flexDirection: 'row', alignItems: 'center', gap: 10, minWidth: 200 },
    colEmail: { flex: 2, minWidth: 200 },
    colPhone: { flex: 1, minWidth: 120 },
